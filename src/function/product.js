...
const search = async (req) => {
    try {
        const data = await MongoData.withMongo('product', () => MongoData.get(req));
...
const load = async (req) => {
    try {
        const data = await MongoData.withMongo('product', () => MongoData.findOne(req));
...
const insert = async (req) => {
    try {
        ...
        await MongoData.withMongo('product', () => MongoData.insert(product));
...
const update = async (req) => {
    try {
        ...
        await MongoData.withMongo('product', () => MongoData.update(req, filter));
...
const deleted = async (req) => {
    try {
        const filter = { productid: req.productid };
        await MongoData.withMongo('product', () => MongoData.deleted(filter));
