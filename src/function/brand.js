...
const search = async (req) => {
    try {
        const data = await MongoData.withMongo('pm_brand', (collection) => MongoData.get(collection, req));
...
const load = async (req) => {
    try {
        const data = await MongoData.withMongo('pm_brand', (collection) => MongoData.findOne(collection, req));
...
const insert = async (req) => {
    try {
        ...
        await MongoData.withMongo('pm_brand', (collection) => MongoData.insert(collection, user));
...
const update = async (req) => {
    try {
        ...
        await MongoData.withMongo('pm_brand', (collection) => MongoData.update(collection, req, filter));
...
const deleted = async (req) => {
    try {
        const filter = { brandid: req.brandid };
        await MongoData.withMongo('pm_brand', (collection) => MongoData.deleted(collection, filter));
...
const getCache = async (req) => {
    try {
        const data = await MongoData.withMongo('pm_brand', (collection) => MongoData.get(collection, {}));
        return new apiresult(false, 'Lấy thông tin thành công!', 'Lấy thông tin thành công!', data);
    } catch (error) {
        return new apiresult(true, 'Lỗi lấy thông tin!', error.message);
    }
