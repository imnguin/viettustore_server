...
const search = async (req) => {
    try {
        const data = await MongoData.withMongo('pm_area', () => MongoData.get(req));
...
const load = async (req) => {
    try {
        const data = await MongoData.withMongo('pm_area', () => MongoData.findOne(req));
...
const insert = async (req) => {
    try {
        ...
        await MongoData.withMongo('pm_area', () => MongoData.insert(user));
...
const update = async (req) => {
    try {
        ...
        await MongoData.withMongo('pm_area', () => MongoData.update(req, filter));
...
const deleted = async (req) => {
    try {
        const filter = { areaid: req.areaid };
        await MongoData.withMongo('pm_area', () => MongoData.deleted(filter));
...
const getCache = async (req) => {
    try {
        const data = await MongoData.withMongo('pm_area', () => MongoData.get({}));
        return new apiresult(false, 'Lấy thông tin thành công!', 'Lấy thông tin thành công!', data);
    } catch (error) {
        return new apiresult(true, 'Lỗi lấy thông tin nhân viên', error.message);
    }
