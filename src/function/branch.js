...
const search = async (req) => {
    try {
        const data = await MongoData.withMongo('pm_branch', () => MongoData.get(req));
...
const load = async (req) => {
    try {
        const data = await MongoData.withMongo('pm_branch', () => MongoData.findOne(req));
...
const insert = async (req) => {
    try {
        ...
        await MongoData.withMongo('pm_branch', () => MongoData.insert(user));
...
const update = async (req) => {
    try {
        ...
        await MongoData.withMongo('pm_branch', () => MongoData.update(req, filter));
...
const deleted = async (req) => {
    try {
        const filter = { branchid: req.branchid };
        await MongoData.withMongo('pm_branch', () => MongoData.deleted(filter));
...
const getCache = async (req) => {
    try {
        const data = await MongoData.withMongo('pm_branch', () => MongoData.get({}));
        return new apiresult(false, 'Lấy thông tin thành công!', 'Lấy thông tin thành công!', data);
    } catch (error) {
        return new apiresult(true, 'Lỗi lấy thông tin nhân viên', error.message);
    }
